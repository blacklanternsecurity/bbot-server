...
class TestAppletEvents(BaseAppletTest):
    ...
    async def after_scan_2(self):
        ...
        dns_events = [e async for e in self.bbot_server.get_events(type="DNS_NAME")]
        assert all(e.type == "DNS_NAME" for e in dns_events)
        assert len(dns_events) == 22

        # filter events by host
        host_events = [e async for e in self.bbot_server.get_events(host="t2.tech.evilcorp.com")]
        ...
        host_events = [e async for e in self.bbot_server.get_events(host="evilcorp.com")]
        ...
        host_events = [e async for e in self.bbot_server.get_events(host="com")]
        assert host_events == []

        # filter events by domain
        domain_events = [e async for e in self.bbot_server.get_events(domain="com")]
        assert all(e.host.endswith(".com") for e in domain_events)
        print(f".com events: {len(domain_events)}")

        domain_events = [e async for e in self.bbot_server.get_events(domain="evilcorp.com")]
        assert all(e.host.endswith(".evilcorp.com") or e.host == "evilcorp.com" for e in domain_events)
        print(f".evilcorp.com events: {len(domain_events)}")

        domain_events = [e async for e in self.bbot_server.get_events(domain="tech.evilcorp.com")]
        assert all(e.host.endswith(".tech.evilcorp.com") or e.host == "tech.evilcorp.com" for e in domain_events)
        print(f".tech.evilcorp.com events: {len(domain_events)}")

        domain_events = [e async for e in self.bbot_server.get_events(domain="t1.tech.evilcorp.com")]
        assert all(e.host.endswith(".t1.tech.evilcorp.com") or e.host == "t1.tech.evilcorp.com" for e in domain_events)
        print(f".t1.tech.evilcorp.com events: {len(domain_events)}")
