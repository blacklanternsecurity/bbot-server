...
class DNS_Links(BaseApplet):
    ...
    async def ingest_event(self, asset: Asset, event: Event) -> list[AssetActivity]:
        ...
        if event.type == "DNS_NAME":
            ...
            old_dns_records = self._flatten_dns_records(old_dns_records)
            new_dns_records = getattr(event, "dns_children", {}) or {}
            new_dns_records = self._flatten_dns_records(new_dns_records)
            removed_dns_records = old_dns_records - new_dns_records
            added_dns_records = new_dns_records - old_dns_records
