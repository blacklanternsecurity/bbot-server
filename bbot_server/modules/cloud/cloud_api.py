...
class CloudApplet(BaseApplet):
    ...
    async def cloudcheck(self, host: str) -> list[dict[str, str]]:
        ...
        result = []
        providers = await self._cloudcheck.lookup(host)
        for provider in providers:
            # Extract provider name from the result
            provider_name = provider.get("name", "Unknown")
            # Get tags (like 'cloud', 'cdn', 'waf')
            tags = provider.get("tags", [])
            provider_type = tags[0] if tags else "unknown"
            # For backwards compatibility, we'll use the hostname as "belongs_to"
            result.append({"provider": provider_name, "provider_type": provider_type, "belongs_to": host})
        return result
    ...
    async def cloud_providers_stats(
        ...
        stats = {}
        query = AssetQuery(domain=domain, target_id=target_id, fields=["cloud_providers"])
